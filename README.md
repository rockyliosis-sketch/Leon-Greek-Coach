...
3. **🏛️ 学生端 9 大实战模块闭环**
   - 包含：连线配对、拼字大作战、智能选择题、正误判断题、希译中输入、中译希输入、真题挑战、单词表艾宾浩斯复习、**单元语法与情景特训（v2.0 升级 4 大多维题型与 30 题日课）**。
4. **🔍 家长后台 (Admin Dashboard) 全景课纲穿透透视**
   - 单元教研标签（`🎯 语法攻坚`、`🗣️ 情境交际`、`📚 核心词汇`）；
   - 支持一键打开 `🔍 查看教学重点、变位与黄金句型 →` 深度弹窗查阅。
5. **🛡️ 全题型出题前双向自检保险系统 (Pre-Flight QA & Verification Gate)**
   - **双向互检机制**：以“用答案验证题目，用题目验证答案”为铁律，在题库注入前执行句式还原、语义对齐、干扰项互斥与正字法校验。
   - **选项绝对置乱**：严防首项固定漏洞，确保选择题答案在四个选项位置中呈离散随机分布。
   - **一键自动化门禁**：内置 `python3 scripts/curriculum_qa_verifier.py` 及专用 Agent Skill [`greek-curriculum-qa-validator`](file:///Users/johnsmacbook/.gemini/config/skills/greek-curriculum-qa-validator/SKILL.md)，全面阻断瑕疵题目流入生产环境。详细规范请参阅 [📄 出题前自检保险系统规范](docs/CURRICULUM_QA_INSURANCE_SYSTEM.md)。
