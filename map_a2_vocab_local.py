...
# Unit keywords for semantic mapping (Units 31 to 45)
unit_keywords = {
...
def main():
    ...
    # 2. Local semantic mapping for the remainder
    unmapped_words = [w for w in words if w["raw"] not in mapped_words]
    print(f"Step 2: Classifying {len(unmapped_words)} words locally via keywords...")
    ...
    print(f"Locally classified via keywords: {semantic_count} words.")
    
    # 3. Fallback distribution for the remaining words
    remaining_words = [w for w in unmapped_words if w["raw"] not in keyword_mapping]
    print(f"Step 3: Distributing remaining {len(remaining_words)} words sequentially across Units 31-45...")
    ...
    print(f"\nSuccessfully wrote {len(compiled_words)} A2 words to {VOCAB_JSON_PATH}.")
